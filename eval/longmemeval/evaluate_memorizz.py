...
class LongMemEvalEvaluator:
    ...
    def _load_dataset(self):
        ...
        try:
            dataset_name = f"zhuang13/LongMemEval"
            if self.dataset_variant == "oracle":
                dataset = load_dataset(dataset_name, "oracle")
            elif self.dataset_variant == "s":
                dataset = load_dataset(dataset_name, "s")
            elif self.dataset_variant == "m":
                dataset = load_dataset(dataset_name, "m")
            else:
                raise ValueError(f"Unknown dataset variant: {self.dataset_variant}")
            
            logger.info(f"Loaded LongMemEval-{self.dataset_variant.upper()} dataset")
            return dataset["test"]
    ...
    def _process_conversation_history(self, agent: MemAgent, history: List[Dict[str, Any]]) -> None:
        """
        Process conversation history session by session to build up agent memory.
        
        Args:
            agent: The Memorizz agent
            history: List of conversation sessions
        """
...
"""
        ...
        try:
            ...
            eval_response = self.eval_model.generate_response(evaluation_prompt)
